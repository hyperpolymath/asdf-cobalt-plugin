...
TOOL_NAME="cobalt"
TOOL_REPO="cobalt-org/cobalt.rs"
BINARY_NAME="cobalt"
...
get_download_url() {
  ...
  local asset_name
  asset_name="$(echo 'cobalt-v{version}-{arch}-{os}.tar.gz' | sed "s/{version}/$version/g" | sed "s/{os}/$os/g" | sed "s/{arch}/$arch/g")"
  ...
  asset_name="$(echo 'cobalt-v{version}-{arch}-{os}.tar.gz' | sed "s/{version}/$version/g" | sed "s/{os}/$os_lower/g" | sed "s/{arch}/$arch/g")"
